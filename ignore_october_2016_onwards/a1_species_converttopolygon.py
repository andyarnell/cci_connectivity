...
wkspce2 =  "C:/Data/cci_connectivity/scratch/spp/raster".format(percentThresh)

wkspce3 =  "C:/Data/cci_connectivity/scratch/spp/vector".format(percentThresh)
...
print "Looping through rasters projecting to coord system of forest and storing here {0} and here {1} \n".format(wkspce2,wkspce3)
...
for rst in rstList[0:]:
    ...
    newRst = SetNull(Con((Raster(inMemRst)==1),1,0)==0,1)
    #newRst.save(rst)
    ...
    try:
        ...
        arcpy.AddField_management(fc,"spp_id","TEXT")
        arcpy.CalculateField_management(in_table=fc, field="spp_id",expression='"'+(rst[:-4])+'"',expression_type="PYTHON",code_block="#")
        arcpy.AddField_management(fc,"patchID","LONG")
        arcpy.CalculateField_management(in_table=fc,field="patchID",expression="!FID!+1",expression_type="PYTHON",code_block="#")
