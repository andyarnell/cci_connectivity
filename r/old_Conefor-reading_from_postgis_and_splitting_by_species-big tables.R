...
#Note: can choose specific runs for different corridors by choosing the id number from the development (e.g. fid_dev number) - for these see development file
strSQL="(
select distinct foo1.id_no1, foo1.season, foo1.count from 
(select id_no, id_no1, season::int, count (distinct (node_id)) 
from  cci_2017.int_grid_pas_trees_40postcent_30agg_by_nodeids_t1 group by id_no, id_no1,season order by count desc) 
as foo1,
(select distinct id_no1, season from cci_2017.int_grid_pas_trees_40postcent_30agg_by_nodeids_t1 
/*where impacted =-1 and not impacted = 1*/ /* and fid_dev=6*/) 
as foo2
where 
foo1.count>1
...
select id_no, id_no1, season::int, count (distinct (node_id)) 
from  
cci_2017.int_grid_pas_trees_40postcent_30agg_by_nodeids_t1 
...
as foo1
left join 
(select distinct id_no1, season from cci_2017.int_grid_pas_trees_40postcent_30agg_by_nodeids_t1 where impacted = 1) 
as foo2
