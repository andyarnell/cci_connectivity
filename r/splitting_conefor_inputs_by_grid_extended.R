...
for (i in 1:length(x)){
  ...
  for (j in 1:nrow(cells)){
    ...
    #both.nodes<-unique(rbind(from.nodes,to.nodes))
    str(both.nodes)
    #for (k in 1:length(both.nodes)){
    k=0
    while (length(both.nodes)=>1){
      print (k)
      k=k+1
       #from.rows<-in.data[which(in.data[1]==both.nodes[k])]
       from.rows<-subset(in.data,in.data$V1==both.nodes[k])
       #print(from.rows)
#       #keep only first 3 cols (no ned for grid cell ids)
       #from.rows<-cbind(from.rows[1],from.rows[2],from.rows[3])
       to.rows<-subset(in.data,in.data$V2==str(both.nodes[k]))
       #print(to.rows)
...
#       ##then append those in the "to" grid id column, 
       ...
       zero.dist1<-subset(from.rows[1],both.rows$V3==0)
       zero.dist2<-subset(to.rows[2],both.rows$V3==0)
       zero.dist<-c(zero.dist1,zero.dist2)
       #both.nodes<-
       #subset(zero.dist,zero.dist!==is.na)
       #zero.dist<-zero.dist[which(zero.dist==is.na)]
      }
#       print(both.rows)
